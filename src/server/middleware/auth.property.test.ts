...
describe('Property 2: Expired Token Rejection', () => {
  ...
  it('should reject all JWTs with expiry timestamps in the past', () => {
    fc.assert(
      fc.property(userPayloadArb, pastOffsetSecondsArb, (payload, pastOffsetSeconds) => {
        ...
        const token = jwt.sign({ ...payload, iat: issuedAt, exp: expiredAt }, config.jwtSecret);
  ...
  it('should accept all JWTs with expiry timestamps in the future', () => {
    ...
    fc.assert(
      fc.property(userPayloadArb, futureOffsetSecondsArb, (payload, futureOffsetSeconds) => {
        // Create a token that expires `futureOffsetSeconds` from now
        const now = Math.floor(Date.now() / 1000);
        const expiresAt = now + futureOffsetSeconds;
        const issuedAt = now;

        const token = jwt.sign({ ...payload, iat: issuedAt, exp: expiresAt }, config.jwtSecret);
